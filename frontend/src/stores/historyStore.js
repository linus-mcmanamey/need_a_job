...
export const useHistoryStore = defineStore('history', () => {
  ...
  /**
   * Paginated applications (client-side pagination)
   */
  ...
  /**
   * Total pages based on filtered applications
   */
  const totalPages = computed(() => {
    return Math.ceil(filteredApplications.value.length / pagination.value.pageSize)
  })
  ...
  async function updateFilters(newFilters) {
    ...
    // Reset to page 1 when filters change (important for pagination)
    pagination.value.page = 1

    console.log('[HistoryStore] Filters updated, reset to page 1')
  ...
  /**
   * Set current page (for client-side pagination, no API call)
   * @param {number} page - Page number
   */
  function setPage(page) {
    if (page < 1 || page > totalPages.value) {
      console.warn(`[HistoryStore] Invalid page number: ${page}`)
      return
    }

    pagination.value.page = page
    console.log(`[HistoryStore] Changed to page ${page} of ${totalPages.value}`)
  }
